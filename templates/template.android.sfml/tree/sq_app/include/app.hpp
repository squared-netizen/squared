...
//
// Note what is NOT here: no Android header, no SFML header, no EGL, no GLES.
// This class is portable — the platform layer in sq_android/ calls into it, and
// the same class compiles unchanged under template.android.cpp, which reaches
// Android through NativeActivity instead of SFML. Keeping both Android and SFML
// out of sq_app/ is what makes that true, and it is worth defending as you add
// code.
...
///
/// Under this template that thread is SFML's, not Android's UI thread: SFML
/// receives the lifecycle callbacks on the UI thread and runs the platform
/// layer on a thread it owns. Blocking here will therefore not trigger an ANR
/// the way it would under template.android.cpp — but it still stops drawing and
/// stops draining events, so the rule is the same: do not block.
class App {
public:
    App();
    ~App();
