# Squared Time Package Agent Instructions
...
Provides pausable and scaled time domains plus a bounded deterministic deadline queue.
...
- root programmer page: `docs/programmer/squared-time/README.md`;
- root developer page: `docs/developer/squared-time/README.md`.
...
Document `Timepiece`, clock/time aliases, pause, scale, update/tick behavior, conversions, `DeadlineQueue`, handles, scheduling, cancellation, capacity/error behavior, and examples.
...
Document time-domain state transitions, monotonic assumptions, deadline ordering, queue structure and complexity, cancellation, deterministic update behavior, bounds, and Messaging integration.
...
Keep Time portable and independent. Messaging consumes Time; Time must not depend on Messaging.
...
`docs/developer/squared-time/` for:

- Timepiece pause/scale/update states;
- deadline schedule/cancel/fire flow;
- Time-to-Messaging dependency;
...
Run `squared_time_test` plus Messaging delayed-delivery tests.
