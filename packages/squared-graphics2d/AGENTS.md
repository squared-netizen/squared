# Squared Graphics2D Package Agent Instructions
...
Provides portable 2D rendering contracts and data types used by applications and GUI.
...
- root programmer page: `docs/programmer/squared-graphics2d/README.md`;
- root developer page: `docs/developer/squared-graphics2d/README.md`.
...
Document `Texture`, `TextureRegion`, `TextureAtlas`, atlas regions, `Sprite`, `SpriteBatch`, and `OrthographicCamera`, including ownership, moves, UV/coordinate conventions, dimensions, lookup failures, batching lifecycle, and examples.
...
Document resource handles and lifetimes, non-owning regions, atlas parsing, page ownership, sprite batching, camera matrices, complexity, transactionality, and backend implementation points.
...
Depend only on portable Graphics and Math contracts. Backend implementations belong in the backend package.
...
`docs/developer/squared-graphics2d/` for:

- texture-atlas page/region ownership;
- SpriteBatch begin/draw/end sequence;
- camera-to-batch transform flow;
...
Run `squared_graphics2d_portable_test`, graphics tests, and relevant atlas/showcase tests.
