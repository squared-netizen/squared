# Squared SDL2/OpenGL Backend Package Agent Instructions
...
Implements the selected SDL2/OpenGL ES backend for portable Application, Graphics, and Graphics2D contracts.
...
- root programmer page: `docs/programmer/squared-backend-sdl2-opengl/README.md`;
- root developer page: `docs/developer/squared-backend-sdl2-opengl/README.md`.
...
Document document user-visible backend selection, required assets/configuration, context capabilities, resource creation failures, and any intentionally public adapter entrypoint; implementation-only source symbols stay developer-only.
...
Document SDL lifecycle, OpenGL context ownership, texture upload and destruction, atlas/image loading, batching, coordinate conversion, error translation, and Android/SDL boundaries.
...
This package implements portable contracts. Never make the portable packages depend on SDL or OpenGL headers.
...
Run the portable graphics tests plus the backend-dependent GUI showcase or Android smoke workflow when available.
