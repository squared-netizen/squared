...
`sq::gui::Ui` owns one widget tree. Its public input boundary accepts
...
```cpp
sq::gui::ButtonGroup difficulty(1, 1);
auto casual = std::make_unique<sq::gui::RadioButton>("Casual", true);
auto expert = std::make_unique<sq::gui::RadioButton>("Expert");
difficulty.add(*casual);
difficulty.add(*expert);

auto save = std::make_unique<sq::gui::Button>("Save", save_game);
save->set_glyph("S", skin.font("icons"));
auto loading = std::make_unique<sq::gui::ProgressBar>(0.0F, 100.0F, 35.0F);
```
...
```cpp
auto mount = std::make_unique<sq::gui::Button>("Mount", mount_disk);
mount->set_tooltip("Mount the selected HoloDisk cartridge");

sq::gui::TooltipConfig tooltip_config;
...
sq::gui::ButtonStyle action = skin.button_style("default");
...
```cpp
sq::gui::Ui ui(width, height, std::move(skin));
auto column = std::make_unique<sq::gui::LinearLayout>();
column->add(std::make_unique<sq::gui::Label>("HoloDisk"));
column->add(std::make_unique<sq::gui::Button>("Mount", mount_disk));
...
```cpp
auto form = std::make_unique<sq::gui::Table>();
form->add(std::make_unique<sq::gui::Label>("Name"))
    .align(sq::gui::Alignment::end);
form->add(std::make_unique<sq::gui::TextField>())
    .grow_x().fill_x();
form->row();
form->add(std::make_unique<sq::gui::Separator>())
    .column_span(2).grow_x().fill_x();

auto window = std::make_unique<sq::gui::Window>("Character");
