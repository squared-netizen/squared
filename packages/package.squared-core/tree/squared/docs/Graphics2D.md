...
Applications render through portable `sq::graphics2d` contracts.
...
- The independent Squared Graphics module provides
  `sq::graphics::Context`, which owns the window, context, viewport,
  clear, and presentation operations.
- `sq::graphics2d::Texture` owns a backend texture.
- `sq::graphics2d::TextureRegion` references a rectangle within a texture.
- `sq::graphics2d::TextureAtlas` owns one or more page textures and
  exposes named regions.
- `sq::graphics2d::BitmapFont` owns portable BMFont metrics and safe
  relative texture-page references.
- `sq::graphics2d::GlyphLayout` converts UTF-8 text into page-indexed,
  SpriteBatch-ready glyph placements.
- `sq::graphics2d::Sprite` stores a region, transform, and color.
- `sq::graphics2d::SpriteBatch` batches ordered textured quads.
- `sq::graphics2d::OrthographicCamera` maps logical 2D coordinates to the
  framebuffer.
