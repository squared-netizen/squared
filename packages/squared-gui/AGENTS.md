# Squared GUI Package Agent Instructions
...
Provides the portable retained-mode widget library, layouts, skins, input/focus handling, windows/dialogs, and transactional libGDX skin loading.
...
- root programmer page: `docs/programmer/squared-gui/README.md`;
- root developer page: `docs/developer/squared-gui/README.md`.
...
Document every geometry/style type, `Painter`, `Drawable`, `Skin`, widget and container, `Cell`, `Window`, `Dialog`, `Ui`, skin-loader input/result/error types, callbacks, ownership transfers, reference lifetimes, input coordinates, exceptions, and examples.
...
Document Composite widget ownership, measure/layout algorithms, focus and pointer capture, event propagation, modal overlays, painter strategy, drawable lifetimes, atlas resolution, transactional skin loading, validation, memory bounds, and UTF-8 behavior.
...
GUI depends on portable Application, Scene2D, Graphics, and Graphics2D contracts. It does not depend on SDL, Android, OpenGL, HoloDisk, or another widget hierarchy.
...
Run `squared_gui_test`, `squared_gui_showcase_test`, package-doc tests, and the applicable Android showcase build.
